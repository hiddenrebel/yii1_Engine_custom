...
class StaticpageController extends BackEndController
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['StaticPage']))
		{
			...
			if ($filename = CUploadedFile::getInstance($model,'img_page')) {

				$file_name_save= md5(date("Y-m-d h:i:s")).'-'.$filename->getName();
				$model->img_page = $file_name_save;
				if (!is_dir(Yii::app()->params['images'].'/Static/')) {
					mkdir(Yii::app()->params['images'].'/Static/',0777);
				}
				$filename->saveAs(Yii::app()->params['images'].'/Static/'.$file_name_save);
			}
			if($model->save())
				$this->redirect(array('view','id'=>$model->id_page));
		}
	...
	{
		$static = $this->loadModel($id);
		$image = Yii::app()->params['images'].'/Static/'.$static->img_page;
		if (file_exists($image) && !is_dir($image))
            unlink($image);
        $static->delete();
